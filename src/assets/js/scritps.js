...
const Storage = {
  chave: 'dev.finances:transactions',
  get() {
    return JSON.parse(localStorage.getItem(Storage.chave)) || [];
  },
  set(transactions) {
    localStorage.setItem(Storage.chave, JSON.stringify(transactions));
  },
};
...
const Transaction = {
  ...
  incomes() {
    ...
    for (const item of Transaction.all) {
      if (item.amount < 0) {
        income += item.amount;
      }
    }
...
const App = {
  init() {
    for (const [index, item] of Transaction.all.entries()) {
